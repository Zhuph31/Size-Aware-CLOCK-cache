 ...
 * ClockHandInteger: just an optional optimization to reduce memory
 * consumption when cache size is equal to or less than 255,65535,4B-1,...
 */
...
public:
  ...
  inline const LruValue get(const LruKey &key) noexcept {
    LruValue value = accessClock2Hand(key, nullptr);
  ...
  LruValue const accessClock2Hand(const LruKey &key, const LruValue *value,
    ...
    } else // could not found key in cache, so searching in circular-buffer
           // starts
    {
      // first check threshold
      if (key.size() > threshold_.get_threshold()) {
        return loadData(key);
      }
