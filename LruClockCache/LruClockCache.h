...
public:
  ...
  inline const std::vector<LruValue>
  getMultiple(const std::vector<LruKey> &key) noexcept {
  ...
  // then cache is available from RAM on next get/set access with same key
  inline void set(const LruKey &key, const LruValue &val) noexcept {
  ...
  void flush() {
    std::lock_guard<std::mutex> lg(mut);
    for (auto mp = mapping.cbegin(); mp != mapping.cend() /* not hoisted */;
         /* no increment */) {
